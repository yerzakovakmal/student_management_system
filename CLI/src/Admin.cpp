...
void Admin::loadStudents() {
    ...
    for (int i = 0; i < count; i++) {
        //temporary - readBinary overwrites
...
void Admin::loadProfessors() {
    ...
    in.read(reinterpret_cast<char*>(&count), sizeof(count));
    for (int i = 0; i < count; i++) {
        Professor* p = new Professor("__load__", "__load__@x.com", "placeholder", "UNKNOWN");
...
void Admin::createProfessor(string name, string email, string password, string dept) {
    if (password.size() < 6)
        throw ValidationException("Password must be at least 6 characters.");
    if (dept.empty())
...
Student* Admin::findStudentByEmail(string email) const {
    for (int i = 0; i < (int)students.size(); i++)
...
Professor* Admin::findProfessorByEmail(string email) const {
    for (int i = 0; i < (int)professors.size(); i++)
        if (professors[i]->getEmail() == email) 
            return professors[i];
    return NULL;
}
...
void Admin::addCourse(string courseId, string name, int credits, int capacity, string days, string time) {
    ...
    courses.push_back(Course(courseId, name, credits, capacity, days, time));
    saveCourses();
...
void Admin::assignProfessorToCourse(string profId, string courseId) {
    ...
    for (int i = 0; i < (int)courses.size(); i++) {
        if (courses[i].courseId == courseId) {
            ...
            saveCourses();
            saveProfessors();
            cout << "[Admin] " << p->getName() << " assigned to course " << courseId << endl;
            return;
        }
    }
    cout << "[Admin] Course ID '" << courseId << "' not found." << endl;
    ...
    for (int i = 0; i < (int)courses.size(); i++) {
        if (courses[i].courseId == courseId) {
            ...
            if (enrolled >= courses[i].capacity) {
                cout << "[Admin] Course '" << courseId << "' is full (" << enrolled << "/" << courses[i].capacity << ")." << endl;
                return;
            }
...
void Admin::assignGrade(string studentId, string courseId, double score) {
    ...
    if (score < 0.0 || score > 100.0){
        throw ValidationException("Score " + to_string((int)score) + " is out of range (0-100).");
    }
    ...
    try {
        // Fix GPA Drifting: update by reference inside the Student object
        s->setGrade(courseId, score);
        saveStudents();
        cout << "[Admin] Grade " << score << " assigned to " << s->getName() << " for course " << courseId << endl;
    } catch (ValidationException& ve) {
        cout << "[Admin] Error: " << ve.what() << endl;
    }
}
...
void Admin::viewAllProfessors() const {
    ...
    for (int i = 0; i < (int)professors.size(); i++) {
        cout << "  " << *professors[i]<< "  | Dept: " << professors[i]->getDepartment() << endl;
    }
}
...
void Admin::viewAllCourses() const {
    ...
    for (int i = 0; i < (int)courses.size(); i++) {
        ...
        cout << "  [" << c.courseId << "] " << c.courseName
             ...
             << "  | Professor: " << profName 
             << "  | Schedule: " << c.daysOfWeek << " @ " << c.timeSlot << endl;
    }
}
...
void Admin::saveReport() const {
    ...
    file << "\n-- Students --" << endl;
    for (int i = 0; i < (int)students.size(); i++) {
        file << "  ID: " << students[i]->getUserId() << "  | " << students[i]->getName() << "  | Pass: " << students[i]->getPassword() << "  | GPA: " << fixed << setprecision(2) << students[i]->getGPA() << endl;
    }

    file << "\n-- Professors --" << endl;
    for (int i = 0; i < (int)professors.size(); i++) {
        file << "  ID: " << professors[i]->getUserId() << "  | " << professors[i]->getName() << "  | Pass: " << professors[i]->getPassword() << "  | Dept: " << professors[i]->getDepartment() << endl;
    }

    file << "\n-- Courses --" << endl;
    for (int i = 0; i < (int)courses.size(); i++) {
        const Course& c = courses[i];
        file << "  [" << c.courseId << "] " << c.courseName << "  Credits: " << c.credits << "  Capacity: " << c.capacity;
        if (!c.professorId.empty())
...
void Admin::displayPanel() {
    ...
    while (choice != 0) {
        ...
        cout << "  9.  View user password (by ID)" << endl;
        cout << "  \n--- Courses ---\n" << endl;
        cout << "  10. View all courses" << endl;
        cout << "  11. View course detail" << endl;
        cout << "  12. Add course" << endl;
        cout << "  13. Remove course" << endl;
        cout << "  14. Assign professor to course" << endl;
        cout << "  15. Enroll student in course" << endl;
        cout << "  16. Assign grade to student" << endl;
        cout << "  \n--- System ---\n" << endl;
        cout << "  17. Save system report to file"   << endl;
        cout << "  0.  Logout" << endl;
        cout << "Choice: ";
        cin >> choice;
        cin.ignore();
        ...
        if (choice == 1) {
            ...
            if (s != NULL) 
                cout << "  Found: " << *s << "  | GPA: " << s->getGPA() << endl;
            else           
                cout << "  No student named '" << n << "' found." << endl;
            ...
            if (p != NULL) 
                cout << "  Found: " << *p << "  | Dept: " << p->getDepartment() << endl;
            else           
                cout << "  No professor named '" << n << "' found." << endl;

        } else if (choice == 9) {
            string id;
            cout << "User ID (Student or Prof): ";
            getline(cin, id);

            Student* s = findStudent(id);
            if (s != NULL) {
                cout << "  Student Found: " << s->getName() << " | Password: " << s->getPassword() << endl;
            } else {
                Professor* p = findProfessor(id);
                if (p != NULL) {
                    cout << "  Professor Found: " << p->getName() << " | Password: " << p->getPassword() << endl;
                } else {
            ...
            cout << "Days (e.g. Mon,Wed): ";
            getline(cin, days);

            cout << "Time (e.g. 10:00-11:30): ";
            getline(cin, time);

            try {
                addCourse(cid, cname, cred, cap, days, time);
            } catch (ValidationException& ve) {
                cout << "[Validation Error] " << ve.what() << endl;
            } catch (FileException& fe) {
                cout << "[File Error] " << fe.what() << endl;
